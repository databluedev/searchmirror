...
function GeoCitations() {
  return <Section id="geo" labelledBy="geo-heading">
      <div className="grid gap-14 lg:grid-cols-[1fr_minmax(0,26rem)] lg:items-start lg:gap-20">
        <div>
          ...
          <SectionHeading id="geo-heading">
            The other half of search doesn't rank.
          </SectionHeading>
          <Lede>
          ...
          <dl className="mt-12 grid gap-x-10 gap-y-9 sm:grid-cols-2">
            {SIGNALS.map(([term, description]) => <div key={term}>
  ...
  >
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {ASSISTANTS.map(([label, named]) => <span
    key={label}
    className={"inline-flex items-center gap-1.5 rounded-pill px-2.5 py-1 text-[11px] font-semibold " + (named ? "bg-ink text-white" : "bg-surface text-ink-3 ring-1 ring-inset ring-[color:var(--line)]")}
  >
                  <span
    className={"h-1.5 w-1.5 rounded-pill " + (named ? "bg-white" : "bg-ink-4")}
  />
                  {label}
                </span>)}
            </div>
            ...
            <dl className="mt-6 divide-y divide-line-2 border-y border-line text-[13.5px]">
              {READOUT.map(([term, value, delta, direction]) => <div
    key={term}
  ...
  >
                  ...
                  <dd className="num font-semibold">
                    {value}
                    {delta ? <span
    className={"ml-1.5 align-[0.08em] text-[11px] font-semibold " + (direction === "up" ? "text-up" : "text-down")}
  >
