...
import { GitHubIcon } from "../ui/logo";
...
function Hero() {
  ...
  return <div id="top" className="relative isolate overflow-hidden">
      <BackgroundGrid />
      <Glow className="left-1/2 top-[-12rem] h-[34rem] w-[34rem] -translate-x-1/2" />

      <div className="mx-auto w-full max-w-[1360px] px-6 pb-16 pt-8 md:px-8 md:pb-20 md:pt-12">
    ...
    /* Sized so the second line fits on one line at every breakpoint —
       36px at 375px is the binding case. */
          ...
          <h1 className="mt-6 text-[2.1rem] font-semibold leading-[1.0] tracking-tightest sm:text-[3rem] md:text-[3.5rem] lg:text-[4.25rem]">
       ...
       wrapping identically at any width. */
            ...
            <span aria-hidden="true" className="grid">
              ...
              <span className="[grid-area:1/1]">
                {reduced ? HEADLINE_B : <TypingAnimation
    as="span"
...
function SampleTable() {
  return <div className="relative mx-auto mt-16 w-full max-w-[680px] md:mt-20">
  ...
  >
        ...
        <table className="w-full border-collapse text-left">
          ...
          <tbody>
            {SAMPLE.map(([keyword, position, delta, direction]) => <tr key={keyword} className="border-t border-line-2">
    ...
    /* The sign carries the direction; `--up` / `--down` reinforce
       ...
       — the two rank colours are 2.6:1 and 4.4:1 as text on white. */
                ...
                <td className="num px-5 py-3 text-right text-[13.5px] font-semibold">
                  <span
    className={"mr-1.5 align-[0.08em] text-[9px] " + (direction === "up" ? "text-up" : direction === "down" ? "text-down" : "text-flat")}
  >
