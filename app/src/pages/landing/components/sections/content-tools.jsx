...
function ContentTools() {
  return <Section id="content" labelledBy="content-heading">
      ...
      <ul className="mt-14 max-w-[46rem]">
        {TOOLS.map(({ title, body, needs }) => <li
    key={title}
    className="group flex flex-col rounded-md border border-line bg-surface p-6 transition-all duration-fast ease-brand hover:-translate-y-1 hover:border-accent/40 hover:shadow-[0_14px_32px_-18px_rgba(15,15,16,0.28)]"
  >
            ...
            <span
    className={"mt-6 inline-flex w-fit items-center gap-1.5 rounded-pill px-2.5 py-1 text-[11px] font-semibold " + (needs ? "bg-surface-2 text-ink-3 ring-1 ring-inset ring-[color:var(--line)]" : "bg-ink text-white")}
  >
              <span
    className={"h-1.5 w-1.5 rounded-pill " + (needs ? "bg-ink-4" : "bg-white")}
  />
              {needs ? `Needs ${needs}` : "Runs on your DataBlue key"}
            </span>
          </li>)}
      </ul>
    </Section>;
