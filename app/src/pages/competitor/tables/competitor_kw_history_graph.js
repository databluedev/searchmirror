...
function CompKeywordHistoryChart({ children, ...props }) {
   var series = [];
   var options = {};
   
   // PROPS DATA LOADED HERE
   var linechartkwdata = {"k_r": [], "Mk_r": [], "t_c": 135, "lrd": moment().format("MM-DD-YYYY")};
   var revfulldate = [];
   if(props.kwdata && linechartkwdata.k_r.length === 0){
      // console.log("props.kwdata ", props.kwdata)
      var a = props.kwdata.RK ? props.kwdata.RK : [];
      var b = props.kwdata.MRK ? props.kwdata.MRK : [];
      var key = props.kwdata.key ? props.kwdata.key.toString().split("~")[0] : 0;

      /* A day with no position is a GAP in the series, not a point drawn at
         an out-of-range sentinel. The sentinel was not even the ceiling: at
         the default depth of three pages only the first 30 are searched. */
      const convertranks = a.map(el => (
         el === 0 ? null : el
      ));
      const convertMainranks = b.map(el => (
         el === 0 ? null : el
      ));
      linechartkwdata = {...linechartkwdata, ...props.kwdata, 'k_r':convertranks, 'Mk_r':convertMainranks, 'key':key}
   ...
   const [stateData, setStateData] = useState({
      ...
      mainkwrankfull : linechartkwdata.Mk_r.slice(0,this_weekday),
      mainkwrank : linechartkwdata.Mk_r.slice(0,this_weekday),
   ...
   useEffect(() => {
      ...
      var cgrpid = cookies.get('__sp_cgrp__'); 
      ...
      if(userid && grpid && linechartkwdata.key) {
         ...
         axios.post(global.apiurl + '/compai/keywordgraph', data, { 
   ...
   var { loading, filtervalue, kwdatesfull, kwrank, kwdates, kwrankfull, mainkwrankfull, mainkwrank } = stateData;
   ...
   const handleFilter = (startpos, endpos, value) => {
      ...
      if ((filtervalue !== value || value === "custom_date_range")) { 
         ...
         if (value === "today") {
            ...
            setStateData({...stateData, loading: true, kwrank : [], mainkwrank:[], filtervalue : value});
         } else {
            ...
            if(userid && grpid && linechartkwdata.key) {
               ...
               axios.post(global.apiurl + '/compai/keywordgraph', data, { 
                  ...
                  if(res.status === "true") {
                     ...
                     const convertMainranks = linechartkwdata.Mk_r.map(el => (
                        el === 0 ? null : el
                     ));
                     ...
                     setStateData({
                        ...
                        mainkwrankfull : convertMainranks,
                        mainkwrank : convertMainranks.slice(startpos,endpos),
                        loading : false, 
   ...
   const chartdatachange = (startpos, endpos, value) => { 
      ...
      var mainupdate = mainkwrankfull.slice(startpos, endpos);
      if (update.every( (val, i, arr) => val === arr[0] )) {
         setTimeout(() => {
            setStateData({
               ...stateData, 
               kwrank : update,
               mainkwrank : mainupdate,
   ...
   /* Bounds from days that HAVE a position: Math.max/min coerce null to 0,
      so leaving the gaps in would pin the axis minimum at 0. */
   const plottedRanks = [...(kwrank || []), ...(mainkwrank || [])]
   ...
   series = [{
      name: (props.projectbase && props.projectbase['pn']) ? props.projectbase['pn'] : 'My Rank', 
      type: 'line',
      data: mainkwrank.slice(0).reverse()
   },{
      name: (props.projectbase && props.projectbase.cpn) ? props.projectbase.cpn : 'Competitor Rank', 
      type: 'line',
   ...
   options = {
      ...
      colors: [CHART.series[0], CHART.series[1], CHART.note],
      dataLabels: {
      ...
      markers: {
         // size: 3,
         strokeWidth: 0,
         size: kwrank.length > 92 ? [0.01, 0.01] : kwrank.length === 1 ? [4, 4] : kwrank.length > 45 ? [2,2] : [3, 3],
         colors: [CHART.series[0], CHART.series[1], CHART.note],
      ...
      yaxis: {
         ...
         labels: {
            formatter: function (y) {
               // return y > 100 ? ">30" : (y <= minValue || y < 0) ? "Notes" : y ? y.toFixed(0) + "" : 0 ;
               return y > 100 ? ">30" : y.toFixed(0) + "" ;
            },
            style: {
               colors: CHART.label,
               fontSize: '10px', 
      ...
      tooltip: {
         ...
         followCursor: true,
         x: {
            formatter: function (val, opts) { return val ? moment(kwdatesrev[opts.dataPointIndex]).format("MMM D, YYYY") : moment(kwdates[0]).format("MMM D, YYYY") },
            // formatter: function (val, opts) {  return console.log(val, opts) },
         },
         y: {
            // A gap has no value to format; say so rather than printing a
            // ceiling the keyword was never measured against.
            formatter: function (val, opts) { return (val === null || val === undefined) ? "No position recorded" : val },
            ...
            // formatter: function (val, opts) { return (opts.seriesIndex === 1) ? notes[opts.dataPointIndex] : ((opts.seriesIndex === 0 && val > 100) || (!val && kwrank[0] > 100)) ? ">30" : val ? val : kwrank[0] },
         },
      }, 
      noData: {
         text: loading ? "" : "No enough data",  
         align: 'center',
         verticalAlign: 'top',
         offsetX: 0,
         offsetY: 100,
	...
	return (
      <div className={props.className}> 
         <div id="chart" className={loading? "" : "charthistory"} > 
            ...
            <div className="m-t30">
               <div className="gLoaderLabel"> 
                  ...
                  <ReactApexChart options={options} series={series} type={kwrank.length > 1 ? "line" : "area"} height={400} /> 
               ...
               This dynamic graph shows the change in your keyword rankings versus the change in your competitors' keyword rankings over a period of time. 
               </Para> 
            </div>
         </div>
...
export default CompKeywordHistoryChart;