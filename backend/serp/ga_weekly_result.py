    ...
    try:
        ...
        if hasattr(trackings, "id") and hasattr(trackings, "fk_user_id") and hasattr(trackings, "fk_group_id"):
            ...
            if not ga_refresh_token or not ga_property_id:
                GA_weekly_monitor.objects.filter(id=trackings.id).update(track_status="failed", track_scheduled_start=next_date_to_isodate(str(trackings.track_scheduled_start), 7))
            ...
            if 'rows' in search_analytics_result:
                ...
                for item in rows:
                    ...
                    transformed_data.append({'value': source, 'session': session})
            ...
            GA_weekly_monitor.objects.filter(id=trackings.id).update(track_status="done", track_scheduled_start=next_date_to_isodate(str(trackings.track_scheduled_start), 7))
        else:
            GA_weekly_monitor.objects.filter(id=trackings.id).update(track_status="failed", track_scheduled_start=next_date_to_isodate(str(trackings.track_scheduled_start), 7))
    except Exception as e:
        print(f'error{str(e)}')
...
# @api_view(['GET'])
@cron_only
def ga_weekly_monitor(self):
    try:
        schdld_tracks=GA_weekly_monitor.objects.filter(track_status="scheduled").count()
        inprogress_ga_tracking_count = GA_weekly_monitor.objects.filter(track_status="inprogress").count()

        if inprogress_ga_tracking_count >= MAX_TRACKS:
            return JsonResponse({"status": "true", "dt": "Track Queue Filled!"})
        

        ga_trackings=GA_weekly_monitor.objects.filter(track_status='scheduled').all().order_by("-id")[0:MAX_TRACKS-inprogress_ga_tracking_count]
        ...
        GA_weekly_monitor.objects.filter(id__in=scheduled_tracks).update(track_status="inprogress")
...
# @api_view(['GET'])
@cron_only
def ga_weekly_rescheduler(self):
    try:

        rescheduled_tracks = list()

        tracks_scheduled_before_one_week = GA_weekly_monitor.objects.filter(track_scheduled_start__lt=datetime.now(), track_status__in=["failed", "done"]).all()[0:20]
        ...
        if tracks_scheduled_before_one_week:
            ...
            GA_weekly_monitor.objects.filter(id__in=rescheduled_tracks, track_status__in=["failed", "done"]).update(track_status="scheduled")
...
def out():
    ...
    landing_page_analytics_params = {
        "dateRanges":[{"startDate":i,"endDate":i}],
        "dimensions":[{"name":"landingPage"}],
        "metrics":[{"name":"sessions"}]
    }
