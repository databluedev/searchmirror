...
def ga_last_year_pool(ga_tracks):
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_TRACKS) as taskExecutor:
            taskExecutorResult = taskExecutor.map(ga_last_overview_monthly, ga_tracks)
...
def ga_last_overview_monthly(ga_trackings):
    try:
        if hasattr(ga_trackings, 'id') and hasattr(ga_trackings, 'fk_user_id') and hasattr(ga_trackings, 'fk_group_id'):
            ...
            if not ga_refresh_token or not ga_property_id:
                GA_monthly_reports.objects.filter(id=ga_trackings.id).update(overview_status="failed" )
            ...
            GA_monthly_reports.objects.filter(fk_user_id=ga_trackings.fk_user_id,
            ...
            GA_monthly_reports.objects.filter(id=ga_trackings.id).update(overview_status="done")
        else:
            GA_monthly_reports.objects.filter(id=ga_trackings.id).update(overview_status="failed", )
    except Exception as e:
        print(f'error{str(e)}')
...
@cron_only
def last_year_overview_data_monthly(request):
    try:
        global MAX_TRACKS
        schdld_tracks=GA_monthly_reports.objects.filter(overview_status="scheduled").count()
        inprogress_ga_tracking_count = GA_monthly_reports.objects.filter(overview_status="inprogress").count()

        if inprogress_ga_tracking_count >= MAX_TRACKS:
            return JsonResponse({"status": "true", "dt": "Track Queue Filled!"})
        
        ga_trackings=GA_monthly_reports.objects.filter(overview_status__in=['scheduled', 'failed']).all().order_by("-id")[0:MAX_TRACKS-inprogress_ga_tracking_count]
        if not ga_trackings:
        ...
        GA_monthly_reports.objects.filter(id__in=scheduled_tracks).update(overview_status="inprogress")
