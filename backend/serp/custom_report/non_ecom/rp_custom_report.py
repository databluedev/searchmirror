...
@api_view(['POST','GET']) 
def add_report_gsc_metrics(request): 
    try:
...
@api_view(['POST','GET'])
def add_report_keyword_metrics(request): 
    try:
        userid = request.data['userid'] 
        grpid = request.data['grpid']
...
@api_view(['POST','GET'])
def add_report_domain_metrics(request): 
    try:
        userid = request.data['userid'] 
        grpid = request.data['grpid']
...
@api_view(['POST','GET'])
def add_report_ga_metrics(request): 
    try:
        userid = request.data['userid']
        grpid = request.data['grpid']
        print('CHECK')
...
def test(request):  
    sd = rank_manager_create(uid=7, gid=6) 
    if sd:
        return JsonResponse({'st':1, 'count': sd })   
    else:
        return JsonResponse({'st':0, 'count': sd }) 