...
@api_view(['GET'])
def automation_rematch_call(request, _ustr_, _kstr_): 
	try:
		_base__result_, _setting__data_  = _at__common_.__base_validation__(request, _ustr_)
		if _base__result_ and _kstr_.isdigit(): 
			
			if int(_kstr_)%2 == 1: 
				_cg__active__data_ = CGASearchCategory.objects.filter(category_status="RINIT").values("fk_user_id", "search_category_id", "fk_search_id", "category_name", "fk_self_domain_id", "fk_comp_domain_id", "category_status").order_by('modified_date').first() # ASC ORDER
			else:
				_cg__active__data_ = CGASearchCategory.objects.filter(category_status="RINIT").values("fk_user_id", "search_category_id", "fk_search_id", "category_name", "fk_self_domain_id", "fk_comp_domain_id", "category_status").order_by('-modified_date').first()
			...
			if _cg__active__data_ and "search_category_id" in _cg__active__data_:
				...
				cg_self_match_urls_list = CGASearchMatch.objects.exclude(source_url_id=None).filter(fk_search_category=_cg__active__data_["search_category_id"], match_url_id=None).values_list('source_url_id', flat=True).all()
				cg_comp_match_urls_list = CGASearchMatch.objects.exclude(match_url_id=None).filter(fk_search_category=_cg__active__data_["search_category_id"], source_url_id=None).values_list('match_url_id', flat=True).all()

				
				cg_match_urls_list = list(cg_self_match_urls_list) + list(cg_comp_match_urls_list)
				cg_category_urls_dict = {}
				
				if cg_match_urls_list:
					cg_unmatched_urls = CGADomainUrl.objects.filter(domain_url_id__in=cg_match_urls_list).values('domain_url_id', 'fk_domain_id', 'source_url').all()

					if cg_unmatched_urls:
						for each_url_value in cg_unmatched_urls:				
							if str(each_url_value['fk_domain_id']) not in cg_category_urls_dict:
								cg_category_urls_dict[str(each_url_value['fk_domain_id'])] = []
							cg_category_urls_dict[str(each_url_value['fk_domain_id'])].append(str(each_url_value['domain_url_id'])) 

				if cg_category_urls_dict:
					...
					if len(domain_keys) == 2 and str(_cg__active__data_['fk_self_domain_id']) in domain_keys and str(_cg__active__data_['fk_comp_domain_id']) in domain_keys: 
						...
						if cg_self_urls_list and cg_comp_urls_list:
							...
							save_path = os.getcwd()+"/project/files/cga/url_rematch/cm_"+str(_cg__active__data_["search_category_id"])+"_"+time_value+".txt"  
							save_to_file(cg_category_output_data, save_path)  

							if search_match_serial_data:
								...
								CGASearchMatch.objects.exclude(source_url_id=None).filter(fk_search_category=_cg__active__data_["search_category_id"], match_url_id=None).delete()
								CGASearchMatch.objects.exclude(match_url_id=None).filter(fk_search_category=_cg__active__data_["search_category_id"], source_url_id=None).delete()
								...
								if cga_matches: 
									...
									return JsonResponse({'st': 1, 'status': "re-mapping completed"}) 

						CGASearchCategory.objects.filter(search_category_id=_cg__active__data_['search_category_id']).update(category_status="DONE")
						return JsonResponse({'st': 1, 'status': "No re-mapping has found"}) 

					CGASearchCategory.objects.filter(search_category_id=_cg__active__data_['search_category_id']).update(category_status="FAIL")
					return JsonResponse({'st': 1, 'status': "mapping not completed"}) 
			else:
				return JsonResponse({'st': 0, 'status': "No Process were available for re-mapping"}) 
