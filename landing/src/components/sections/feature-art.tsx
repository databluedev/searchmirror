...
/**
 * Small product-detail illustrations, built from the interface's own fragments
 * rather than stock art. All decorative: each is aria-hidden and the card's
 * heading and sentence carry the meaning.
 *
 * The three comparison hues are the app's categorical chart palette
 * (DESIGN.md > Charts > Identity), which is why they are literal here.
 */
  ...
  return (
    <Frame>
      <div className="w-full space-y-2.5">
        {rows.map(([width, position, tone], i) => (
          <div key={i} className="flex items-center gap-3 rounded-sm bg-surface px-3 py-2">
 ...
 *  chip is the reader's own domain, which is the state the feature exists to
 *  report. */
export function AiOverviewArt() {
...
export function CompareBarsArt() {
        ...
        <div className="flex justify-between gap-1.5">
          {days.map((day, i) => (
            <span
              key={i}
              className={
                "flex h-7 flex-1 items-center justify-center rounded-sm text-[11px] font-semibold " +
                (i === 0
                  ? "bg-ink text-white"
...
export function RolesArt() {
  ...
  return (
    <Frame>
      <div className="w-full space-y-2">
        {members.map(([initials, role, owner]) => (
          <div
            key={initials}
          ...
          >
            ...
            <span
              className={
                "ml-auto rounded-pill px-2 py-0.5 text-[10px] font-semibold " +
                (owner
                  ? "bg-ink text-white"
